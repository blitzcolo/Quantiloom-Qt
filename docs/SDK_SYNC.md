...
**本文中的提交号(`5181035`、`B2`、`F6` 等)均指 Quantiloom-dev 仓库**,
文件路径除非另有说明也指该仓库;本仓库内的路径写作 `src/...`、`assets/...`。
...
**现状** 这与 dev 侧刚修掉的 G1 是同一个错误,根因也相同:`Types.hpp` 里
...
也不能与 CLI 的结果逐位比对。这一条在 dev 侧记为 F6。
...
也没有任何检查会发现两边不一致。这与 dev 侧 B2 的成因相同 —— 烘焙产物与实际加载
路径之间隔着一个无人记录的手工步骤。
