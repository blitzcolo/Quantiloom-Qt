...
This repo contains **no physics** — it links the prebuilt SDK at `../Quantiloom-SDK/windows_amd64` (currently `H:/Quantiloom-SDK/windows_amd64`, CMake cache var `QUANTILOOM_SDK_ROOT`; the path is derived from this checkout's location, never hard-coded to a drive) via `find_package(Quantiloom)`. The SDK is produced by the sibling core repo `../Quantiloom-dev`. Consequences:

- **Core changed → refresh flow (in this order):**
  1. `cd /mnt/h/Quantiloom-dev && ./build_wsl.sh` — rebuilds core and reinstalls the SDK
  2. `cd /mnt/h/Quantiloom-Qt && cmake.exe --build build-msvc --config Release -j` — relink against the new `.lib`; POST_BUILD steps copy `Quantiloom.dll`, `SpectraForge.dll`, Qt DLLs, and `.spv` shaders from the SDK into `build-msvc/Release/`
...
```bash
cd /mnt/h/Quantiloom-Qt/build-msvc/Release && ./QuantiloomQt.exe
```
