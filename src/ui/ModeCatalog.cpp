...
/// Every translatable string in this file shares one context so the glossary
/// applies uniformly and lupdate groups them together.
QString trc(const char* source) {
    return QCoreApplication::translate("ModeCatalog", source);
}
...
QString debugCategoryName(DebugCategory category) {
    switch (category) {
        case DebugCategory::Geometry:    return trc("Geometry");
        case DebugCategory::Material:    return trc("Material");
        case DebugCategory::Lighting:    return trc("Lighting");
        case DebugCategory::BRDF:        return trc("BRDF");
        case DebugCategory::IBL:         return trc("IBL");
        case DebugCategory::Spectral:    return trc("Spectral");
        case DebugCategory::Infrared:    return trc("Infrared");
        case DebugCategory::Diagnostics: return trc("Geometry Diagnostics");
    }
    return {};
}
...
QString debugModeName(DebugVisualizationMode mode) {
    switch (mode) {
        case DebugVisualizationMode::None:
            return trc("None (Normal Rendering)");

        // Geometry
        case DebugVisualizationMode::WorldPosition:   return trc("World Position");
        case DebugVisualizationMode::GeometricNormal: return trc("Geometric Normal");
        case DebugVisualizationMode::ShadedNormal:    return trc("Shaded Normal");
        case DebugVisualizationMode::Tangent:         return trc("Tangent");
        case DebugVisualizationMode::UV:              return trc("UV Coordinates");
        case DebugVisualizationMode::MaterialID:      return trc("Material ID");
        case DebugVisualizationMode::TriangleID:      return trc("Triangle ID");
        case DebugVisualizationMode::Barycentric:     return trc("Barycentric Coords");

        // Material
        case DebugVisualizationMode::BaseColor:      return trc("Base Color (Albedo)");
        case DebugVisualizationMode::Metallic:       return trc("Metallic");
        case DebugVisualizationMode::Roughness:      return trc("Roughness");
        case DebugVisualizationMode::NormalMapDelta: return trc("Normal Map Delta");
        case DebugVisualizationMode::Emissive:       return trc("Emissive");
        case DebugVisualizationMode::Alpha:          return trc("Alpha");

        // Lighting
        case DebugVisualizationMode::NdotL:     return trc("N dot L");
        case DebugVisualizationMode::NdotV:     return trc("N dot V");
        case DebugVisualizationMode::DirectSun: return trc("Direct Sun");
        case DebugVisualizationMode::Diffuse:   return trc("Diffuse");
        case DebugVisualizationMode::AtmosphericTransmittance:
            return trc("Atmospheric Transmittance");

        // BRDF
        case DebugVisualizationMode::FresnelF0: return trc("Fresnel F0");
        case DebugVisualizationMode::Fresnel:   return trc("Fresnel");
        case DebugVisualizationMode::BRDF_Full: return trc("Full BRDF");
        case DebugVisualizationMode::SpecularD: return trc("Specular D (GGX)");
        case DebugVisualizationMode::SpecularG: return trc("Specular G (Smith)");

        // IBL
        case DebugVisualizationMode::ReflectionDir:  return trc("Reflection Direction");
        case DebugVisualizationMode::PrefilteredEnv: return trc("Prefiltered Environment");
        case DebugVisualizationMode::BrdfLut:        return trc("BRDF LUT");
        case DebugVisualizationMode::IblSpecular:    return trc("IBL Specular");
        case DebugVisualizationMode::SkyAmbient:     return trc("Sky Ambient");

        // Spectral
        case DebugVisualizationMode::XYZ_Tristimulus: return trc("XYZ Tristimulus");
        case DebugVisualizationMode::BeforeChromaCorrection:
            return trc("Before Chroma Correction");
        case DebugVisualizationMode::SpectralReflectance550:
            return trc("Spectral Reflectance @550nm");
        ...
        // the same word.
        case DebugVisualizationMode::Temperature:  return trc("Surface Temperature");
        case DebugVisualizationMode::IREmissivity: return trc("IR Emissivity");
        case DebugVisualizationMode::IREmission:   return trc("IR Emission");
        case DebugVisualizationMode::IRReflection: return trc("IR Reflection");

        // Diagnostics
        case DebugVisualizationMode::VertexPositions: return trc("Vertex Positions (Hash)");
        case DebugVisualizationMode::IndexValues:     return trc("Index Values");
        case DebugVisualizationMode::InstanceID:      return trc("Instance ID");
        case DebugVisualizationMode::PrimitiveID:     return trc("Primitive ID");
        case DebugVisualizationMode::IndexBufferPos:  return trc("Index Buffer Position");
        case DebugVisualizationMode::V0Position:      return trc("V0 Position");
        case DebugVisualizationMode::RawIdx0:         return trc("Raw idx0");
        case DebugVisualizationMode::V0Raw:           return trc("V0 Raw (clamped)");
...
QString debugModeDescription(DebugVisualizationMode mode) {
    switch (mode) {
        case DebugVisualizationMode::None:
            return trc("Standard rendering output. No debug visualization.");

        // Geometry
        case DebugVisualizationMode::WorldPosition:
            return trc("World-space hit position. RGB = fractional XYZ coordinates.");
        case DebugVisualizationMode::GeometricNormal:
            return trc("Raw geometric normal from triangle vertices (before normal mapping).");
        case DebugVisualizationMode::ShadedNormal:
            return trc("Final shading normal after interpolation and normal map application.");
        case DebugVisualizationMode::Tangent:
            return trc("Tangent vector for normal mapping. Used for TBN matrix construction.");
        case DebugVisualizationMode::UV:
            return trc("Texture coordinates. RG = fractional UV, useful for texture mapping debug.");
        case DebugVisualizationMode::MaterialID:
            return trc("Material index visualized as distinct colors. Each material gets unique color.");
        case DebugVisualizationMode::TriangleID:
            return trc("Primitive (triangle) index. Useful for mesh topology inspection.");
        case DebugVisualizationMode::Barycentric:
            return trc("Barycentric coordinates within triangle. RGB = weights at 3 vertices.");

        // Material
        case DebugVisualizationMode::BaseColor:
            return trc("Albedo/base color from texture or material parameters.");
        case DebugVisualizationMode::Metallic:
            return trc("Metallic parameter. 0 = dielectric, 1 = metal.");
        case DebugVisualizationMode::Roughness:
            return trc("Roughness parameter. 0 = mirror smooth, 1 = fully rough.");
        case DebugVisualizationMode::NormalMapDelta:
            return trc("Normal map perturbation from surface normal.");
        case DebugVisualizationMode::Emissive:
            return trc("Emissive color/intensity. Self-illumination without external lighting.");
        case DebugVisualizationMode::Alpha:
            return trc("Alpha/opacity value. 1 = opaque, 0 = transparent.");

        // Lighting
        case DebugVisualizationMode::NdotL:
            return trc("Dot product of normal and light direction. Basic diffuse term.");
        case DebugVisualizationMode::NdotV:
            return trc("Dot product of normal and view direction. Affects Fresnel and specular.");
        case DebugVisualizationMode::DirectSun:
            return trc("Direct sunlight contribution after shadowing and attenuation.");
        case DebugVisualizationMode::Diffuse:
            return trc("Diffuse lighting term: kD * albedo * NdotL.");
        case DebugVisualizationMode::AtmosphericTransmittance:
            return trc("Atmospheric transmittance factor from scattering/absorption LUT.");

        // BRDF
        case DebugVisualizationMode::FresnelF0:
            return trc("Base reflectivity at normal incidence. Depends on metallic and IOR.");
        case DebugVisualizationMode::Fresnel:
            return trc("Fresnel reflectance at current viewing angle (Schlick approximation).");
        case DebugVisualizationMode::BRDF_Full:
            return trc("Complete Cook-Torrance BRDF evaluation: D * G * F / (4 * NdotL * NdotV).");
        case DebugVisualizationMode::SpecularD:
            return trc("GGX/Trowbridge-Reitz normal distribution function.");
        case DebugVisualizationMode::SpecularG:
            return trc("Smith geometry/masking-shadowing function.");

        // IBL
        case DebugVisualizationMode::ReflectionDir:
            return trc("Mirror reflection direction for environment map sampling.");
        case DebugVisualizationMode::PrefilteredEnv:
            return trc("Pre-filtered environment map sample at current roughness level.");
        case DebugVisualizationMode::BrdfLut:
            return trc("BRDF integration LUT sample. RG = scale and bias for split-sum.");
        case DebugVisualizationMode::IblSpecular:
            return trc("Final IBL specular contribution: prefiltered * (F * scale + bias).");
        case DebugVisualizationMode::SkyAmbient:
            return trc("Ambient sky lighting contribution (diffuse IBL).");

        // Spectral
        case DebugVisualizationMode::XYZ_Tristimulus:
            return trc("CIE XYZ tristimulus values from spectral integration. Before RGB conversion.");
        case DebugVisualizationMode::BeforeChromaCorrection:
            return trc("Linear RGB before chromaticity correction. May show color shifts.");
        case DebugVisualizationMode::SpectralReflectance550:
            return trc("Material spectral reflectance sampled at 550 nm (green reference).");

        // Infrared
        case DebugVisualizationMode::Temperature:
            return trc("Surface temperature in Kelvin. Blue = cold, red = hot (colormap).");
        case DebugVisualizationMode::IREmissivity:
            return trc("IR emissivity factor. 1 = perfect blackbody, 0 = perfect reflector.");
        case DebugVisualizationMode::IREmission:
            return trc("Thermal emission contribution: emissivity * Planck(T, lambda).");
        case DebugVisualizationMode::IRReflection:
            return trc("IR reflection of ambient thermal radiation.");

        // Diagnostics
        case DebugVisualizationMode::VertexPositions:
            return trc("Hash of 3 vertex positions. The same face should show similar colors. "
                       "Different colors on one face mean index corruption.");
        case DebugVisualizationMode::IndexValues:
            return trc("Triangle vertex indices as RGB (normalized by 32). For a cube: idx 0-23.");
        case DebugVisualizationMode::InstanceID:
            return trc("TLAS instance index. Verifies instance-to-geometry mapping.");
        case DebugVisualizationMode::PrimitiveID:
            return trc("PrimitiveIndex() value. R = id/12 (gradient), G = alternating, B = even/odd. "
                       "For a cube: 12 distinct triangles with a smooth R gradient.");
        case DebugVisualizationMode::IndexBufferPos:
            return trc("Index buffer read position. R = basePos/36, G = offset/36, B = primID/12. "
                       "For a single BLAS, G should be 0.");
        case DebugVisualizationMode::V0Position:
            return trc("First vertex (v0) position mapped to 0-1 using frac(). "
                       "For a ±1 cube: 0 for both +1 and -1, 0.5 for 0.");
        case DebugVisualizationMode::RawIdx0:
            return trc("Raw idx0 value. R = idx0/32, G = readAddr/32, B = offset/32. "
                       "For a cube: R should be 0-0.72 (idx 0-23). G = R when offset is 0.");
        case DebugVisualizationMode::V0Raw:
            return trc("v0 position clamped (not frac). -1 maps to 0, 0 to 0.5, +1 to 1. "
                       "For a cube: only 0 or 1, never 0.5.");

        default:
            return trc("Unknown debug mode.");
    }
}
...
QString spectralModeName(SpectralMode mode) {
    switch (mode) {
        ...
        case SpectralMode::Single:     return trc("Single Wavelength");
...
QString spectralModeDescription(SpectralMode mode) {
    switch (mode) {
        case SpectralMode::RGB:
            return trc("Fast RGB rendering, no spectral integration. "
                       "Best for real-time preview.");
        case SpectralMode::VIS_Fused:
            return trc("32-wavelength spectral integration with CIE XYZ color matching. "
                       "Physically accurate but slower.");
        case SpectralMode::Single:
            return trc("Monochromatic rendering at a single wavelength. "
                       "Useful for spectral analysis and wavelength-specific effects.");
        case SpectralMode::NIR_Fused:
            return trc("Near-Infrared (%1). Reflected solar radiation, "
                       "vegetation analysis, and night vision.").arg(fusedRangeText(mode));
        case SpectralMode::SWIR_Fused:
            return trc("Short-Wave Infrared (%1). Moisture detection, "
                       "material identification, and imaging through haze.").arg(fusedRangeText(mode));
        case SpectralMode::MWIR_Fused:
            return trc("Mid-Wave Infrared (%1). Thermal imaging for hot objects, "
                       "engine exhaust, and fire detection.").arg(fusedRangeText(mode));
        case SpectralMode::LWIR_Fused:
            return trc("Long-Wave Infrared (%1). Thermal imaging for room-temperature "
                       "objects, people, and buildings.").arg(fusedRangeText(mode));
        default:
            return trc("Unknown spectral mode.");
    }
}
...
QString qualityPresetLabel(const QualityPreset& preset) {
    ...
    QString name;
    if (id == "preview")         name = trc("Preview");
    else if (id == "draft")      name = trc("Draft");
    else if (id == "medium")     name = trc("Medium");
    else if (id == "high")       name = trc("High");
    else if (id == "veryhigh")   name = trc("Very High");
    else if (id == "production") name = trc("Production");
    else                         name = id;

    return trc("%1 (%2 spp)").arg(name).arg(preset.spp);
}
